...
const Driver = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        response.data.bookings.forEach((booking) => {
          if (booking.staffVehicle && booking.driver) {
            vehicleMap.set(
              booking.driver._id,
              booking.staffVehicle.vehicleNumber
            );
          }
        });
    ...
    Promise.all([fetchBookings(), fetchDrivers()]).finally(() =>
      setLoading(false)
    );
  }, []);
  ...
  return (
    <div className="min-h-screen text-white flex flex-col items-center md:w-[78%] md:absolute md:top-[50px] md:left-[259px]">
      <div className="container-fluid w-full max-w-none h-full" >
        {/* Search Bar */}
        <div className=" mb-6 px-2 h-5px" style={{ width: screen < 1000 ? 'full' : '100%', marginTop: screen < 1000 ? '40px' : '20px' }} >
          <input
            type="text"
            placeholder="Search by name, phone, or vehicle number..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full h-[60px] p-[3px] rounded-md border border-gray-600 bg-gray-800 text-white focus:outline-none focus:border-blue-400 overflow-hidden md:block "
          />
        </div>
          ...
          <div className="grid mt-[20px] grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 px-2 w-full">
            {filteredDrivers.map((driver) => {
              ...
              return (
                <div
                  key={driver._id}
                  className="bg-gray-200 rounded-lg p-4 flex flex-col items-center shadow-md hover:shadow-lg transition-shadow w-full"
                  style={{ marginLeft: screen < 1000 ? "0" : "15px", width: screen < 1000 ? '300px' : '100%' }}
                >
                  {/* Driver Image */}
                  <img
